...
TEST_DIR="/tmp/smelter-test-oemdrv"
rm -rf "$TEST_DIR"
mkdir -p "$TEST_DIR"
chmod 777 "$TEST_DIR"
...
sudo virt-install \
    ...
    --extra-args "console=ttyS0,115200 root=live:CDLABEL=CDROM rd.live.image azl.autoinstall inst.ks=/run/initramfs/installer-config.ks" \
    --graphics none \
